...
def pad_zeros_to_chunk(
    sequences: torch.Tensor, chunk_len: int, count_fix=0
) -> torch.Tensor:

    # 判断是 1D 还是 2D
    if sequences.dim() == 1:
        N = sequences.shape[0] + count_fix
        pad_len = (chunk_len - (N % chunk_len)) % chunk_len
        if pad_len > 0:
            sequences = torch.nn.functional.pad(
                sequences, (pad_len, 0), mode="constant", value=0
            )
...
def pad_zeros_to_chunk_list(
    sequences: list[list[int]], chunk_len: int, count_fix=0
) -> list[list[int]]:
    if not sequences:
        return []

    # 找出当前最大长度
    max_len = max(len(seq) for seq in sequences) + count_fix

    # 计算补零后的目标长度，使其能被 chunk_len 整除
    total_len = ((max_len + chunk_len - 1) // chunk_len) * chunk_len

    # 在每个序列前面补零，使其长度达到 total_len
    padded_sequences = []
    for seq in sequences:
        pad_len = total_len - len(seq)
