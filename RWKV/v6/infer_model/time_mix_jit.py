...
class TimeMix(torch.jit.ScriptModule):
    ...
    def forward(self, x, layer_state):
        ...
        r, k, v, w, g, shift_state_out = self.pre_forward(x, layer_state.tmix_shift_states)

        layer_state.tmix_wkv_states = layer_state.tmix_wkv_states.to(r.dtype)

        x = RUN_CUDA(B, T, C, H, r, k, v, w, u=self.time_faaaa, s=layer_state.tmix_wkv_states)

        x = self.post_forward(x, g)
        layer_state.tmix_shift_states = shift_state_out
